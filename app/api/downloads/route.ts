    ...
    for (const task of tasks) {
      if (task.file_path && fs.existsSync(task.file_path)) {
        try {
          ...
          console.error(`获取文件大小失败: ${task.file_path}`, err);
        }
      }
    }
  } catch (err) {
    console.error('更新已下载文件大小失败:', err);
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    `);
    const authors = authorsStmt.all().map((row: any) => row.author_nickname).filter(Boolean);
    ...
    const tasksWithProgress = tasks.map((task: any) => {
      ...
      if (realtimeData && realtimeData.updatedAt > Date.now() - 5000) {
        ...
        return {
          ...
          file_size_formatted: formatFileSize(realtimeData.downloadedBytes),
        ...
        return {
          ...task,
          file_size_formatted: task.file_size ? formatFileSize(task.file_size) : null,
          content_type: task.type || '未知'
        };
      }
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!video_id) {
      return NextResponse.json({ error: '缺少 video_id 参数' }, { status: 400 });
    }

    // 检查是否已存在下载任务
    const existingTask = db.prepare('SELECT * FROM download_tasks WHERE video_id = ?').get(video_id);
    if (existingTask) {
      return NextResponse.json({ success: true, data: existingTask, message: '任务已存在' });
    }

    return NextResponse.json({ error: '请通过订阅同步创建下载任务' }, { status: 400 });
  } catch (error: any) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // 获取任务完整信息（用于删除文件）
    const task = db.prepare('SELECT * FROM download_tasks WHERE id = ?').get(taskId) as any;
    ...
    if (task.file_path) {
      ...
      if (task.type === '图集') {
        // 图集类型：删除所有相关图片
        console.log(`删除图集任务 ${taskId} 的所有图片文件`);
        ...
        if (match) {
          ...
          if (fs.existsSync(task.file_path)) {
            try {
              ...
              console.log(`已删除文件: ${task.file_path}`);
            } catch (err) {
              console.error(`删除文件失败: ${task.file_path}`, err);
        ...
        // 视频类型：删除单个文件
        console.log(`删除视频任务 ${taskId} 的文件: ${task.file_path}`);
      ...
      try {
        if (fs.existsSync(fileDir)) {
          ...
          if (remainingFiles.length === 0) {
            ...
            console.log(`已删除空目录: ${fileDir}`);
          }
        }
      } catch (err) {
        console.error(`删除目录失败: ${fileDir}`, err);
      }
    }
