...
import { formatFolderName, formatFileName } from "@/lib/naming";
    ...
    `,
        ...
        if (task.type === '图集') {
          ...
          if (!task.all_download_urls) {
            return NextResponse.json({ error: "图集下载链接不存在" }, { status: 400 });
          }

          // 解析所有图片 URL
          const imageUrls = task.all_download_urls.split(',').map((url: string) => url.trim()).filter(Boolean);

          if (imageUrls.length === 0) {
            return NextResponse.json({ error: "图集没有有效的下载链接" }, { status: 400 });
          }
          ...
          // 获取下载目录
          const downloadDir = settings.download_dir || path.join(process.cwd(), "downloads");

          // 获取命名配置
          const folderNameFormat = settings.folderNameFormat || "{uid}_{nickname}_{type}";
          ...
          try {
            ...
            for (let i = 0; i < imageUrls.length; i++) {
              ...
              const imagePath = path.join(targetDir, `${fileNamePrefix}_${imageIndex}.${ext}`);

              logger.debug(`下载图片 ${imageIndex}/${imageUrls.length}: ${imageUrl.substring(0, 100)}...`);
              ...
              // 更新进度
              const progress = Math.round((imageIndex / imageUrls.length) * 100);
              db.prepare("UPDATE download_tasks SET progress = ? WHERE id = ?").run(progress, task_id);

              logger.debug(`图片 ${imageIndex}/${imageUrls.length} 下载完成: ${stats.size} bytes`);
            }

            // 更新任务状态为完成，保存第一张图片路径
            db.prepare(
              "UPDATE download_tasks SET status = ?, progress = ?, file_path = ?, file_size = ?, completed_at = ? WHERE id = ?",
            ).run("completed", 100, downloadedFiles[0], totalSize, Date.now(), task_id);

            logger.info(`图集下载完成: ${downloadedFiles.length} 张图片，总大小 ${totalSize} bytes`);
